...
public class TimeManager extends TimeUtils {
    ...
    public boolean validWakeTime(Time time) {
        if (IConfig.getMorningCheck().equals(Time.DISABLED)) return true;
        Time morningTime = IConfig.getMorningCheck();
    ...
    private boolean sleptLate(Time time) {
        if (IConfig.getLateCheck().equals(Time.DISABLED)) return false;
        Time lateTime = IConfig.getLateCheck();
